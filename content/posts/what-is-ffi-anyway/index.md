...
<!--more-->
It’s always annoying when you rely on a complex multi-part system and some obscure part fails. It leaves you groping for answers and inventing unnecessary hack solutions. The dreaded old Stack Overflow rabbit hole. FFI stands for [Foreign Function Interface](https://en.wikipedia.org/wiki/Foreign_function_interface), a mechanism that allows code written in one language to be called by another language.
...
So why doesn’t it want to run on M1? Most solutions suggest to force install the x64 version which can then be run with Rosetta. I’m loathe to try this though, as it might be I get stuck with a suboptimal version of `ruby-ffi` going forward.{{% sidenote "sn-1" %}}I suppose one would forcibly uninstall the x64 `ruby-ffi` and then reinstall the arm64 version.{{% /sidenote %}} That said, Jekyll seems pretty moribund at this point, and so it might be worth experimenting just to get the local build working. My qualms aren’t going to matter if I’m going to use a more modern tool like [Eleventy](https://www.11ty.dev "Link to the Eleventy website") in the future.